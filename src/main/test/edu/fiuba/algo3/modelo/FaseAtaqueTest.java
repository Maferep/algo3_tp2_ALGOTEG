...
public class FaseAtaqueTest {
    ...
    public void test01JugadorAtacaYConquistaDosPaisesDelOtroJugador() throws Exception {
        ...
        ITurno turno = new TurnoMock();
        List<Pais> p = Arrays.asList(atacante1, defensor1);
        FaseAtacar fase = new FaseAtacar(turno, p);
        fase.ataqueFalso(ataqueFalso);
        ...
        List<Pais> pNew = Arrays.asList(atacante2, defensor2);
        FaseAtacar faseNew = new FaseAtacar(turnoNew, pNew);
        faseNew.ataqueFalso(ataqueFalsoNew);
